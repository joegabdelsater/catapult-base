                ...
                <form action="{{ route('catapult.field.store', ['modelId' => $model->id]) }}" method="POSt">
                    ...
                    <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5 mb-4">
                        ...
                        <div class="grid gap-4 mb-4 sm:grid-cols-2">
                            ...
                            @if($model->packages && in_array('spatie_sluggable', $model->packages)) 
                                <div class="flex items-center pt-6">
                                    <input id="sluggable" type="checkbox"  name="sluggable"
                                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                                    <label for="sluggable"
                                        class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Sluggable</label>
    ...
    <script>
        ...
        function updateFormFieldDynamicInputs(selectedType) {
            ...
            if (selectedType === 'text_input') {
                ...
                formFieldDynamicInputs.innerHTML += getCheckboxField('Checked', 'admin_field_config[checked]')
            } else if (selectedType === 'toggle') {
                formFieldDynamicInputs.innerHTML += getTextField('Label', 'admin_field_config[label]', 'Enter label')
                formFieldDynamicInputs.innerHTML += getTextField('On Value', 'admin_field_config[on_value]', 'Enter on value')
                formFieldDynamicInputs.innerHTML += getTextField('Off Value', 'admin_field_config[off_value]',
                    'Enter off value')
                formFieldDynamicInputs.innerHTML += getCheckboxField('Checked', 'admin_field_config[checked]')
            } else if (selectedType === 'rich_editor') {
                formFieldDynamicInputs.innerHTML += getTextField('Label', 'admin_field_config[label]', 'Enter label')
                formFieldDynamicInputs.innerHTML += getTextField('Height', 'admin_field_config[height]', 'Enter height')
                formFieldDynamicInputs.innerHTML += getCheckboxField('Required', 'admin_field_config[required]')
            } else if (selectedType === 'markdown_editor') {
                formFieldDynamicInputs.innerHTML += getTextField('Label', 'admin_field_config[label]', 'Enter label')
                formFieldDynamicInputs.innerHTML += getTextField('Height', 'admin_field_config[height]', 'Enter height')
                formFieldDynamicInputs.innerHTML += getCheckboxField('Required', 'admin_field_config[required]')
