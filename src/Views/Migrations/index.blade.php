    ...
    <div class="p-8 container mx-auto">
        ...
        @foreach ($models as $model)
            <div
                ...
                <a href="{{ route('catapult.migrations.create', ['model' => $model->id]) }}">{{ $model->name }}.php</a>
                <div class="flex flex-row items-center">
                    @if ($model->migration && !$model->migration->created)
                        <span class="text-xs text-rose-500 mr-2 group-hover:text-white">Migration exists but not
                            created</span>
                        <form action="{{ route('catapult.migrations.destroy', ['migration' => $model->migration->id])}}" method="POST">
                            @csrf
                            @method('DELETE')
