...
@section('content')
    <div class="grid grid-cols-2 h-screen p-8">
        <div>
            <h1 class="text-2xl font-bold mb-4">Let's create some controllers!</h1>
            <form class="max-w-lg" method="POST" action="{{ route('catapult.controllers.store') }}">
                @csrf
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <label for="name"
                        class="peer-focus:font-medium absolute text-sm text-gray-500  duration-300 transform -translate-y-6 scale-75 top-2 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Controller
                        name</label>
                </div>
        ...
        <div class="">
            <div class="flex flex-row items-center justify-between mb-2">
                <h1 class="text-2xl font-bold mb-4">Controllers</h1>
                <a href="{{ route('catapult.controllers.generate-all') }}"
                    onclick="return confirm('Are you sure you want to generate all controllers? This will overwrite any existing files.')"
            ...
            <div class="">
                @foreach ($controllers as $controller)
                    <div class="flex justify-between items-center p-3 border-b border-gray-200">
                        ...
                        <form method="POST" action="{{ route('catapult.controllers.destroy', ['controller' => $controller->id]) }}">
                            @csrf
                            <button
                                onclick="return confirm('Are you sure you want to delete this controller from the database?')">
